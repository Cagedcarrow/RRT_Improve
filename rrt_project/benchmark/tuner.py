...
def tune_from_failures(config: PlannerConfig, failures: List[str]) -> Tuple[PlannerConfig, Dict[str, float]]:
    ...
    if c.get("collision_stuck", 0) / total > 0.25:
        next_cfg = replace(
            next_cfg,
            bridge_bias=min(0.5, next_cfg.bridge_bias + 0.05),
    ...
    if c.get("timeout", 0) / total > 0.25:
        next_cfg = replace(next_cfg, goal_bias=min(0.25, next_cfg.goal_bias + 0.02))
        change_log["goal_bias"] = next_cfg.goal_bias

        schedule = []
        for p, a in next_cfg.alpha_schedule:
            schedule.append((p, min(1.0, a + 0.05 if p <= 0.6 else a)))
        next_cfg = replace(next_cfg, alpha_schedule=schedule)
        change_log["alpha_schedule_updated"] = 1.0

    if c.get("overlong_path", 0) / total > 0.25:
        next_cfg = replace(next_cfg, step_size=max(0.02, next_cfg.step_size * 0.9))
        change_log["step_size"] = next_cfg.step_size

    if c.get("unstable", 0) / total > 0.20:
